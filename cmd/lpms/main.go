...
func getOpts(opts *core.LPMSOpts) {
...
func main() {
	...
	glog.Infof("Setting working directory %v", fmt.Sprintf("%v/.tmp", dir))
	opts := core.LPMSOpts{}
	getOpts(&opts)
	lpms := core.New(&opts)
	...
	glog.Infof("Setting publish handle")
	...
	glog.Infof("Setting HLS play handle")
	lpms.HandleHLSPlay(
		//getMasterPlaylist
	...
	glog.Infof("Setting RTMP play handle")
	lpms.HandleRTMPPlay(
		//getStream
